"""Per-model dispatch for the SAE-feature detector pipeline (scripts/rank_sae_features.py,
scripts/validate_sae_features.py): which provider's `load_sae` to use, which layers to pool
candidates from, and (for scripts/rank_sae_features.py's attribution-patching ranking only)
